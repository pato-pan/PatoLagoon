...
default='--cookies cookies.cok --embed-metadata --embed-thumbnail --embed-chapters --sub-langs all,-live_chat,-rechat -c'
...
function findremoved() {
	...
	local archive="$3"
	local tracking="$4"
	rm "${idlists}"/offline.txt
	rm "${idlists}"/found.txt
	if [ $tracking ]; then
		yt-dlp -s ${target} 2>&1 >/dev/null | perl -pe "s/(${logscleaner})//g" | sed -r '/^\s*$/d' | tee "${idlists}"/offline.txt # detector of deleted videos. To get full logs, which also serves as a progress bar, replace "2>&1 >/dev/null" with "2> >(tee >(cat 1>&2) pipes)". Full logs are the same as download, so they are limited by default. Warbo stackoverflow 45798436
		found=$(comm -1 -2 <(sort "${archive}") <(sort offline.txt)); echo "$found" > "${idlists}"/found.txt # Gets only the deleted files that had been downloaded previously. This is unnecessary, and it only reduces the amount of commands, the log size(arguably), and it insignificantly decreases time.
	elif [ -z $tracking ]; then
		yt-dlp --download-archive offline.txt --force-download-archive --flat-playlist -s ${target} >/dev/null 2>&1 # lists every video the channel has. To get full logs, which also serves as a progress bar, remove >/dev/null 2>&1. There's no logs by default because this is similar to your download command.
			...
			id=${f%]*} # removes everything after the last ]
   			id=${id##*[} # removes everything before the last [
				...
				:
	   			...
	   			rm "${f%%.*}.temp.m4a"
				rm "${f%%.*}.webp"
				yt-dlp  --download-archive ${archive} --cookies cookies.cok --force-overwrites --embed-thumbnail --embed-chapters --sub-langs all,-live_chat,-rechat -c ${besta} $id -o "${parent}/${nameformat}"
				success=$?
       				#ffmpeg -i "$f" "${convset[@]}" compat/"${nemu%.m4a}".flac
				ffmpeg -i "$f" "${convset[@]}" compat/"${nemu%.m4a}".m4a #I know adding m4a here is redundant. It should only be just $f instead. This is only here for consistency.
...
# echo collect thumbnails. Currently, no nice and short way to do it without doubling the amount of lines in your script. Here's the command I use
cd "${idlists}"/thumbs
