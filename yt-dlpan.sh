...
function findremoved() {
	...
	if [ $(( $(date +%s) - ($(date +%s -r "${parent}"/preserving/found.txt)+0) )) -le 7889238 ]; then # Currently set to skip if it was last run less than 3 months ago. Necessary since this makes too many requests and takes too much time. This can only be calculated in seconds if you want to change this.
		...
		if [ -z $archive ]; then
			...
			yt-dlp --download-archive offline.txt --force-write-archive --flat-playlist -s ${target} >/dev/null # lists every video the channel has. To get full logs, which also serves as a progress bar, remove >/dev/null.		found=$(comm -2 -3 <(sort -u "${archive}" "${idlists}/thumbs/${archive}") <(sort offline.txt)); echo "$found" > found.txt # if a video is not on the channel, but you have it downloaded, it's assumed that it has been removed.
		fi
